    ...
    $('#codigo_repuesto').on('keydown', function(event) {
        ...
        if (event.keyCode === 13) { 
            ...
            $.ajax({
                ...
                method: 'POST',
                data: { codigo_repuesto: codigo_repuesto,
...
$(document).ready(function() {
    var csrftoken = getCookie('csrftoken'); 
    $('#codigo_responsable').on('keydown', function(event) {

        if (event.keyCode === 13) {
            event.preventDefault();
            var codigo_responsable = $(this).val();
    
            $.ajax({
                url: autocompletar_responsableURL,
                method: 'POST',
                data: { codigo_responsable: codigo_responsable,
                    csrfmiddlewaretoken: csrftoken
                },
                success: function(response) {
                    console.log(response);
                    console.log(response.data);
                    $('#nombres_responsable').val(response.data.nombres_responsable);
                    $('#apellidos_responsable').val(response.data.apellidos_responsable);
                }
            });
        }
    });
...
$(document).ready(function() {
    $('form').on('submit', function(event) {
        ...
        var cantidad_salida_repuesto = $('#cantidad_salida_repuesto').val();
        var codigo_responsable = $('#codigo_responsable').val();
        var fecha_salida_repuesto = $('#fecha_salida_repuesto').val();
        ...
        $.ajax({
            ...
            data: {
                ...
                cantidad_salida_repuesto: cantidad_salida_repuesto,
                codigo_responsable: codigo_responsable,
                fecha_salida_repuesto: fecha_salida_repuesto
            },
            success: function(response) {
